...
namespace Editor_de_Grafos
{
    class Algoritmo
    {
        ...
        */
        ...
        // parent[] 
        public List<int> printMST(int[] parent, int[,] graph, int NumeroNodos)
        {
            List<int> Prim = new List<int>();
            string CadAux = "Relación \t  Peso Relación \n";
            //Prim.Add(parent[1] + 1);
            for (int i = 1; i < NumeroNodos; i++)
            {
                CadAux += (parent[i] + 1) + " - " + (i + 1) + "\t     " + (graph[i, parent[i]]) + "\n";
                Prim.Add(parent[i] + 1);
                Prim.Add(i + 1);
            }
        ...
        public List<List<int>> primMST(int[,] graph, int NumeroNodos, int NodoSeleccionado)
        {
            ...
            int[] parent = new int[NumeroNodos];

            // Key values used to pick 
            // minimum weight edge in cut 
            int[] key = new int[NumeroNodos];

            // To represent set of vertices 
            // not yet included in MST 
            bool[] mstSet = new bool[NumeroNodos];
            ...
            for (int count = 0; count < NumeroNodos - 1; count++)
            {
                ...
                // not yet included in MST 
                int u = minKey(key, mstSet, NumeroNodos);
            ...
            List<List<int>> Caminos = new List<List<int>>();
            List<int> Aux = printMST(parent, graph, NumeroNodos);
            if (Aux != null)
            {
                Caminos.Add(Aux);
            }
